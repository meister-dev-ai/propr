    ...
    ICodeInsightHistoryReader? historyReader = null) : ControllerBase
{
    ...
    {
        var configured = configuration.GetValue("CODE_INSIGHTS_MIN_SEALED_PULL_REQUESTS", DefaultMinimumSampleSize);

        // A floor of one, because a threshold of zero would mean "present a metric computed from nothing as
        // precise", which is the exact failure the threshold exists to prevent.
