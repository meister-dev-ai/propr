...
///     <see cref="MeisterDev.ProPR.Domain.Entities.ReviewJob" /> aggregate root).
/// </summary>
public interface IProtocolRecorder
{
    ...
    /// <summary>
    ///     Creates a new protocol record owned by a mention answer, and returns its identifier. Allowed to
    ///     throw, exactly as <see cref="BeginAsync" /> is; the caller decides whether to continue without a
    ///     protocol.
    /// </summary>
    /// <remarks>
    ///     One answer is one model call, so one record covers the job. Closing it with
    ///     <see cref="SetCompletedAsync" /> moves the tokens onto the job's own totals and the client's daily
    ///     usage sample, which is the route the budget scopes read and the reason a mention that skips this
    ///     call spends money nothing can see.
    /// </remarks>
    /// <param name="mentionReplyJobId">The mention answer this protocol belongs to.</param>
    /// <param name="label">Human-readable label for this protocol pass.</param>
    /// <param name="modelId">The effective AI model deployment name used for this pass.</param>
    /// <param name="ct">Cancellation token.</param>
    /// <param name="logicalModelName">The logical-model role that resolved this pass's runtime, captured at the time.</param>
    /// <returns>The new protocol record's <see cref="Guid" />.</returns>
    Task<Guid> BeginForMentionReplyAsync(
