...
namespace MeisterDev.ProPR.ProCursor.Persistence;

/// <summary>
///     EF Core read repository for ProCursor token usage reporting.
/// </summary>
public sealed class ProCursorTokenUsageReadRepository(
    ProCursorOperationalDbContext db,
    IProCursorKnowledgeSourceRepository knowledgeSourceRepository) : IProCursorTokenUsageReadRepository
{
    ...
    public async Task<ProCursorTokenUsageEventsResponse?> GetRecentEventsAsync(
        ...
        CancellationToken ct = default)
    {
        if (await knowledgeSourceRepository.GetByIdAsync(clientId, sourceId, ct) is null)
        {
            return null;
        }
