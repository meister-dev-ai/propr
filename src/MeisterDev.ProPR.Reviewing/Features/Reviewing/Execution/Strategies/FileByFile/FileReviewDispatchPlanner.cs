...
internal sealed class FileReviewDispatchPlanner(
    ...
    Func<bool>? budgetExhaustedSignal = null)
{
    /// <summary>
    ///     How many files this review may work on at once. Reviewing several files in parallel is a licensed
    ///     capability, so without it a review works one file at a time however high the configured concurrency
    ///     is. It is the same rule the worker applies to whole jobs, one level down. A host with no licensing
    ///     service at all, such as the offline harness, keeps the configured value.
    /// </summary>
    private async Task<int> ResolveFileReviewConcurrencyAsync(CancellationToken ct)
    {
        var parallelReviewExecutionEnabled = licensingCapabilityService is null
                                             || await licensingCapabilityService.IsEnabledAsync(
                                                 PremiumCapabilityKey.ParallelReviewExecution,
                                                 ct);

        return ReviewConcurrencyPolicy.Effective(options.MaxFileReviewConcurrency, parallelReviewExecutionEnabled);
    }
        ...
        CancellationToken ct)
    {
        ...
        var selection = ReviewFileSelectionService.SelectFilesForReview(
            allChangedFiles,
            existingResults,
            executionContext.ExclusionRules,
            ReviewPassSignature.ForPasses(executionContext.ReviewPasses));
        ...
        var semaphore = new SemaphoreSlim(await this.ResolveFileReviewConcurrencyAsync(ct));
    ...
    private BudgetSoftCapSummary BuildBudgetSoftCapSummary(IReadOnlyList<string> skippedFiles)
    {
        ...
        // The signal carries the verdict without the figures: the cap and the spend are metered where the
        // completions are priced, and a summary with the bit set and no numbers is honest about that.
        if (budgetExhaustedSignal?.Invoke() == true)
        {
            return new BudgetSoftCapSummary(true, null, null, skippedFiles);
        }
