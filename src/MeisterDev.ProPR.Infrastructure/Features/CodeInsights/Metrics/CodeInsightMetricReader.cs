    ...
    IDbContextFactory<MeisterProPRDbContext>? contextFactory = null) : ICodeInsightMetricReader
{
        ...
        return this.WithDbAsync(
            async db =>
            {
                ...
                return new CodeInsightRejectionReasonBreakdown(
                    Tally(rows.Select(row => row.Reason)),
                    ...
                    rows
                        ...
                        .Select(group => new CodeInsightConcernClassRejections(
                            group.Key,
                            Tally(group.Select(row => row.Reason)),
    ...
    private static Dictionary<CodeInsightRejectionReason, int> Tally(IEnumerable<CodeInsightRejectionReason?> reasons)
    {
        return reasons
