...
/// </remarks>
internal static class AzureOpenAiTransport
{
    /// <summary>What a refusal says when the host supplied no way to reach the network.</summary>
    internal static readonly string NoClientFactory = UnreachableClients.Refusal("Azure OpenAI");
    ...
    internal static HttpClient Client(ProviderEndpoint endpoint, ProviderHttpPurpose purpose)
    {
        ...
        // The endpoint's defaults ride on every request the client sends. A client library composes its own
        // requests and knows nothing about either, so without the handler a gateway's routing header and a
        // key carried as a query parameter reach discovery and not the model call.
        return Client(endpoint.HostContext?.Http, purpose, [new ProviderEndpointDefaultsHandler(endpoint)]);
    }
    ...
    internal static HttpClient Client(
        ...
        IReadOnlyList<DelegatingHandler>? outerHandlers = null)
    {
        return http is null
            ? throw new InvalidOperationException(NoClientFactory)
            : http.Create(purpose, outerHandlers);
    }
}
