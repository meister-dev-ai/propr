...
///     </para>
/// </remarks>
internal static class AzureOpenAiFailures
{
    /// <summary>Reports a failed verification from a client-library failure that carries an HTTP status.</summary>
    /// <param name="exception">The failure the client library raised.</param>
    internal static ProviderVerificationResult Failed(ClientResultException exception)
    {
        ArgumentNullException.ThrowIfNull(exception);

        return DriverFailureMapper.Failed((HttpStatusCode)exception.Status, exception.Message);
    }
    ...
    internal static ProviderFailureVerdict ClassifyRuntimeFailure(Exception exception)
    {
        ArgumentNullException.ThrowIfNull(exception);
