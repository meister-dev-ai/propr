...
/// <summary>
///     No-op implementation of <see cref="IReviewThreadStatusWriter" /> used when <c>ADO_STUB_PR=true</c>.
///     Thread status update calls are logged but no ADO API call is made.
/// </summary>
internal sealed partial class StubAdoThreadClient(ILogger<StubAdoThreadClient> logger) : IReviewThreadStatusWriter
{
    ...
    [LoggerMessage(
        ...
        Message =
            "StubAdoThreadClient: skipped UpdateThreadStatus for PR#{PullRequestId} thread {ThreadId} → '{Status}' (ADO_STUB_PR=true)")]
