...
internal partial class ForgejoRepositoryExclusionFetcher(
    IClientScmConnectionRepository connectionRepository,
    IHttpClientFactory httpClientFactory,
    ILogger<ForgejoRepositoryExclusionFetcher> logger) : IProviderRepositoryExclusionFetcher
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var fileUri = ForgejoConnectionVerifier.BuildApiUri(
            host,
            $"/repos/{repositoryPath}/contents/{Uri.EscapeDataString(ExcludeFilePath)}",
            $"ref={Uri.EscapeDataString(normalizedBranch)}");

        using var request = ForgejoConnectionVerifier.CreateAuthenticatedRequest(fileUri, connection.Secret);
        using var response =
            await httpClientFactory.CreateClient("ForgejoProvider").SendAsync(request, cancellationToken);
        ...
        var payload = await response.Content.ReadFromJsonAsync<ForgejoContentResponse>(cancellationToken)
                      ?? throw new InvalidOperationException("Forgejo exclusion lookup returned an empty payload.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("Forgejo repository exclusion fetches require a client identifier.");
        }
        ...
        {
            throw new InvalidOperationException("Forgejo repository exclusions require personal access token authentication.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var payload = await response.Content.ReadFromJsonAsync<ForgejoRepositoryResponse>(cancellationToken)
        ...
        return NormalizeRepositoryPath(payload.FullName);
    }
