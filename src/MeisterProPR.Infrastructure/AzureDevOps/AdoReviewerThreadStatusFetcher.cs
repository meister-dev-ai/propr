...
public sealed partial class AdoReviewerThreadStatusFetcher(
    ...
    ILogger<AdoReviewerThreadStatusFetcher> logger) : IReviewerThreadStatusFetcher
{
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            foreach (var thread in rawThreads)
            {
                ...
                results.Add(new PrThreadStatusEntry(
                    thread.Id,
    ...
    private async Task<(GitHttpClient GitClient, Guid? AuthorizedIdentityId)> ResolveGitClientAndAuthorizedIdentityAsync(
        string organizationUrl,
        Guid clientId,
        CancellationToken ct)
    {
    ...
    private static string BuildCommentHistory(GitPullRequestCommentThread thread)
    {
        var lines = thread.Comments!
            .Where(c => !c.IsDeleted && !string.Equals(c.CommentType.ToString(), "system", StringComparison.OrdinalIgnoreCase))
    ...
    [LoggerMessage(Level = LogLevel.Warning, Message = "Failed to fetch reviewer thread statuses for PR #{PullRequestId}. Returning empty list.")]
