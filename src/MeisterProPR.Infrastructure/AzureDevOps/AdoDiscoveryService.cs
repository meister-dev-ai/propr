...
public class AdoDiscoveryService(
    ...
    IClientAdoOrganizationScopeRepository organizationScopeRepository) : IAdoDiscoveryService
{
        ...
        CancellationToken ct = default)
    {
        ...
        var (_, _, connection) = await this.ResolveScopeAsync(clientId, organizationScopeId, ct);
        var repositoryId = await this.ResolveRepositoryIdAsync(connection, projectId, sourceKind, canonicalSourceRef, ct);
        ...
        return branches
            ...
            .Select(branchName => new AdoBranchOptionDto(branchName!, string.Equals(branchName, defaultBranch, StringComparison.OrdinalIgnoreCase)))
            .ToList()
            .AsReadOnly();
    }
        ...
        CancellationToken ct)
    {
        ...
        return sourceKind switch
        {
            ...
            ProCursorSourceKind.AdoWiki => await this.ResolveWikiRepositoryIdAsync(connection, projectId, canonicalSourceRef.Value, ct),
            _ => null,
        };
    }
    ...
    {
        var wikis = await this.GetWikisAsync(connection, projectId, ct);
        var wiki = wikis.FirstOrDefault(candidate => string.Equals(candidate.Id.ToString(), wikiId, StringComparison.OrdinalIgnoreCase));
    ...
    protected internal virtual async Task<(ClientAdoOrganizationScopeDto Scope, ClientAdoCredentials? Credentials, Microsoft.VisualStudio.Services.WebApi.VssConnection Connection)> ResolveScopeAsync(
        Guid clientId,
        Guid organizationScopeId,
        CancellationToken ct)
    {
        var scope = await organizationScopeRepository.GetByIdAsync(clientId, organizationScopeId, ct)
        ...
        var credentials = await credentialRepository.GetByClientIdAsync(clientId, ct);
        var connection = await connectionFactory.GetConnectionAsync(scope.OrganizationUrl, credentials, ct);
        return (scope, credentials, connection);
    }
