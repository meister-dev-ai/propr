...
public partial class AdoRepositoryInstructionFetcher(
    ...
    ILogger<AdoRepositoryInstructionFetcher> logger) : IRepositoryInstructionFetcher
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            files = await this.FetchInstructionFilesAsync(organizationUrl, projectId, repositoryId, targetBranch, clientId, cancellationToken);
        }
        catch (Exception ex)
        {
    ...
    [LoggerMessage(EventId = 4001, Level = LogLevel.Debug, Message = "Fetching repository instructions from {RepositoryId} on branch {Branch}")]
    private static partial void LogFetchStarted(ILogger logger, string repositoryId, string branch);

    [LoggerMessage(EventId = 4002, Level = LogLevel.Debug, Message = "Fetched {Count} relevant instruction(s) from {RepositoryId} on branch {Branch}")]
    private static partial void LogInstructionsFetched(ILogger logger, int count, string repositoryId, string branch);

    [LoggerMessage(
        EventId = 4003,
        Level = LogLevel.Debug,
        Message = "Instruction folder .meister-propr/ absent in {RepositoryId} on branch {Branch}; returning empty list")]
    private static partial void LogInstructionFolderAbsent(ILogger logger, string repositoryId, string branch);

    [LoggerMessage(EventId = 4004, Level = LogLevel.Warning, Message = "Failed to fetch repository instructions from {RepositoryId} on branch {Branch}")]
