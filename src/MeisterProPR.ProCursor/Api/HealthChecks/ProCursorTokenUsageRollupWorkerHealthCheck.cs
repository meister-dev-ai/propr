...
{
    private readonly TimeSpan _staleCycleThreshold = TimeSpan.FromSeconds(Math.Max(1, options.Value.RollupPollSeconds) * 2);
    ...
    public Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        ...
        {
            return Task.FromResult(HealthCheckResult.Unhealthy("ProCursor token usage rollup worker appears stuck in its current cycle.", data: data));
        }

        if (worker.LastCycleCompletedAt.HasValue && now - worker.LastCycleCompletedAt.Value > this._staleCycleThreshold)
        {
            return Task.FromResult(HealthCheckResult.Degraded("ProCursor token usage rollup worker has not completed a recent cycle.", data: data));
        }

        return Task.FromResult(HealthCheckResult.Healthy("ProCursor token usage rollup worker is running.", data: data));
    }
}
