...
public sealed partial class ProCursorQueryService
{
    [LoggerMessage(
        ...
        Message = "ProCursor knowledge query unavailable for client {ClientId}: no eligible sources.")]
    private static partial void LogKnowledgeUnavailableNoEligibleSources(ILogger logger, Guid clientId);

    [LoggerMessage(
        Level = LogLevel.Information,
        Message = "ProCursor knowledge query unavailable for client {ClientId}: no ready snapshots.")]
    private static partial void LogKnowledgeUnavailableNoReadySnapshots(ILogger logger, Guid clientId);

    [LoggerMessage(
        Level = LogLevel.Warning,
        Message =
            "Falling back to lexical ProCursor retrieval for client {ClientId} because query embedding generation is unavailable.")]
