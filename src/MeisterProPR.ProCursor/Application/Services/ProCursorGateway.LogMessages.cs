...
public sealed partial class ProCursorGateway
{
    [LoggerMessage(
        ...
        Message =
            "Guided ProCursor source creation failed because organization scope {ScopeId} was not found for client {ClientId}")]
    private static partial void LogGuidedOrganizationScopeMissing(ILogger logger, Guid clientId, Guid scopeId);

    [LoggerMessage(
        Level = LogLevel.Warning,
        Message =
            "Guided ProCursor source creation failed because organization scope {ScopeId} is disabled for client {ClientId}")]
    private static partial void LogGuidedOrganizationScopeDisabled(ILogger logger, Guid clientId, Guid scopeId);

    [LoggerMessage(
        Level = LogLevel.Warning,
        Message =
            "Guided ProCursor source '{CanonicalSourceValue}' is no longer available in project {ProjectId} for client {ClientId}")]
    private static partial void LogGuidedSourceUnavailable(
        ILogger logger,
        Guid clientId,
        string projectId,
    ...
    [LoggerMessage(
        Level = LogLevel.Warning,
        Message =
            "Guided ProCursor branch validation failed for source '{SourceDisplayName}' on client {ClientId}: {Reason}")]
    private static partial void LogGuidedBranchValidationFailed(
        ILogger logger,
        Guid clientId,
