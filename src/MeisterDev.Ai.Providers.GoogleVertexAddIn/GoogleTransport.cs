...
/// </remarks>
internal static class GoogleTransport
{
    ...
    internal static HttpClient Client(IProviderHttpClientFactory? http, ProviderHttpPurpose purpose)
    {
        return http is null
            ? throw new InvalidOperationException(
                "This Google connection was handed no client factory, and a provider family reaches the network "
                + "only through the one the host supplies.")
            : http.Create(purpose);
    }
}
