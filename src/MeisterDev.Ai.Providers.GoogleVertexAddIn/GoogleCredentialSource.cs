...
public sealed class GoogleCredentialSource : IGoogleCredentialSource
{
    ...
    /// <summary>The credential field holding the Google credential document a Vertex token is minted from.</summary>
    public const string ServiceAccountField = "serviceAccountJson";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            // The header carries whatever it is given as though it were a key. A profile configured for a
            // Google credential document holds a private key, so sending it here would put that key on the wire
            // in a header, to an endpoint that cannot use it. The call is refused instead. Saving the same
            // combination is refused too, so reaching here means a row that predates that check.
            if (ProviderVocabulary.ValuesEqual(endpoint.AuthMode, GoogleVertexProviderDriver.GcpAdcAuth))
            {
                throw new InvalidOperationException(
                    "The Gemini API authenticates with an API key, and this connection is configured for a "
                    + "Google credential document, which belongs to a Vertex AI endpoint.");
            }
