...
public sealed partial class ThreadMemoryService
{
    ...
    [LoggerMessage(
        Level = LogLevel.Warning,
        Message = "Failed to record no-op for thread {ThreadId} client {ClientId}")]
    ...
    [LoggerMessage(
        Level = LogLevel.Warning,
        Message = "Failed to retrieve and reconsider for file {FilePath} client {ClientId}")]
    private static partial void LogRetrieveAndReconsiderFailedCore(
        ILogger logger,
        Exception exception,
        string filePath,
        Guid clientId);
    ...
    [LoggerMessage(
        Level = LogLevel.Warning,
        Message = "Memory reconsideration AI call failed; returning draft result unchanged")]
    ...
    [LoggerMessage(
        Level = LogLevel.Warning,
        Message = "Historical duplicate suppression embedding lookup failed for file {FilePath} client {ClientId}")]
    private static partial void LogDuplicateSuppressionEmbeddingFailedCore(
        ILogger logger,
        Exception exception,
        string filePath,
        Guid clientId);
    ...
    [LoggerMessage(
        Level = LogLevel.Warning,
        Message =
            "Historical duplicate suppression repository lookup failed for repo {RepositoryId} PR {PullRequestId} client {ClientId}")]
    private static partial void LogDuplicateSuppressionLookupFailedCore(
        ILogger logger,
        Exception exception,
        string repositoryId,
        int pullRequestId,
    ...
    private static void LogDuplicateSuppressionEmbeddingFailed(
        ILogger logger,
        string? filePath,
        Guid clientId,
        Exception exception)
    ...
    private static void LogDuplicateSuppressionLookupFailed(
        ILogger logger,
        string repositoryId,
        int pullRequestId,
        Guid clientId,
        Exception exception)
    {
        LogDuplicateSuppressionLookupFailedCore(
            logger,
            exception,
            SanitizeForLog(repositoryId),
            pullRequestId,
            clientId);
    }
