...
public sealed partial class ReviewOrchestrationService
{
    [LoggerMessage(
        ...
        Message =
            "Review job {JobId} failed partially ({FailedCount}/{TotalCount} files failed) — re-queuing for retry")]
    ...
    [LoggerMessage(
        Level = LogLevel.Warning,
        Message = "PR #{PrId} is no longer active (status: {Status}) — failing job {JobId}")]
    private static partial void LogPrNoLongerActive(ILogger logger, int prId, PrStatus status, Guid jobId);

    [LoggerMessage(
        Level = LogLevel.Trace,
        Message = "Skipping review for job {JobId} PR#{PrId} — no new commits or replies")]
    private static partial void LogSkippedNoChange(ILogger logger, Guid jobId, int prId);

    [LoggerMessage(
        Level = LogLevel.Information,
        Message = "Skipping review for job {JobId} PR#{PrId} — AI returned empty review")]
    ...
    [LoggerMessage(
        Level = LogLevel.Information,
        Message = "Review job {JobId} was cancelled externally before file review — exiting without AI calls")]
