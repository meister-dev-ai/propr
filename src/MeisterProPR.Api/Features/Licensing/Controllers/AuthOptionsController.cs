...
[ApiController]
public sealed class AuthOptionsController(ILicensingCapabilityService? licensingCapabilityService = null)
    : ControllerBase
{
    ...
    public async Task<IActionResult> GetAuthOptions(CancellationToken ct = default)
    {
        ...
        {
            return this.Ok(new AuthOptionsDto(InstallationEdition.Community, ["password"], []));
        }

        return this.Ok(await licensingCapabilityService.GetAuthOptionsAsync(ct));
    }
}
