...
Review-time code analysis is served by a single abstraction in the `MeisterDev.ProPR.CodeAnalysis` project:
...
- **Tree-sitter backend** (`MeisterDev.ProPR.CodeAnalysis.TreeSitter`) — TS/TSX/JS/Py/Go/Java/Ruby, syntactic.
- **Roslyn-syntax backend** (`MeisterDev.ProPR.CodeAnalysis.Roslyn`) — C#, via `CSharpSyntaxTree.ParseText`
