...
public sealed class UserPasswordControllerTests(UserPasswordControllerTests.UserPasswordApiFactory factory)
    : IClassFixture<UserPasswordControllerTests.UserPasswordApiFactory>
{
    [Fact]
    public async Task ChangePassword_WithValidCurrentPassword_Returns204_UpdatesHashAndRevokesRefreshTokens()
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Post, "/users/me/password");
    ...
    public sealed class UserPasswordApiFactory : WebApplicationFactory<Program>
    {
    ...
    public sealed class InMemoryRefreshTokenRepository : IRefreshTokenRepository
    {
        ...
        {
            var token = this._tokens.FirstOrDefault(t => t.TokenHash == tokenHash && t.IsActive);
