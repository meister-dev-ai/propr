    ...
    : IClassFixture<ReviewJobIntakeIntegrationTests.IntakeApiFactory>
{
    ...
    public sealed class IntakeApiFactory : WebApplicationFactory<Program>
    {
        ...
        public async Task<Guid> InsertCompletedJobAsync()
        {
            using var scope = this.Services.CreateScope();
            var repository = scope.ServiceProvider.GetRequiredService<IJobRepository>();
            var job = new ReviewJob(Guid.NewGuid(), this.ClientId, "https://dev.azure.com/myorg", "my-project", "my-repo", 77, 1);
        ...
        protected override void ConfigureWebHost(IWebHostBuilder builder)
        {
            ...
            builder.ConfigureServices(services =>
            {
                ...
                adoValidator.IsValidAsync(Arg.Is<string>(value => value != "valid-ado-token"), Arg.Any<string?>(), Arg.Any<CancellationToken>()).Returns(false);
                ...
                userRepo.GetByIdWithAssignmentsAsync(Arg.Any<Guid>(), Arg.Any<CancellationToken>()).Returns(Task.FromResult<AppUser?>(null));
