...
public sealed class AnthropicConformanceTests
{
    ...
    ///     the declaration. The Messages protocol is served by gateways and enterprise proxies as well as by the
    ///     vendor, so there is nothing for this family to refuse on account of the host.
    /// </remarks>
    ...
    public void TheKitReportsEveryCheckItRan()
    {
        ...
        Assert.Equal("meisterdev/anthropic", report.Family);
    ...
    {
        var declared = new AnthropicProviderDriver().Declaration.ConformanceInputs;
    ...
    {
        var constructed = typeof(AnthropicProviderDriver).GetConstructor(Type.EmptyTypes);

        Assert.NotNull(constructed);
        Assert.IsType<AnthropicProviderDriver>(constructed.Invoke([]));
    }
    ...
    {
        var drivers = typeof(AnthropicProviderDriver).Assembly
            .GetExportedTypes()
            .Where(type => type is { IsClass: true, IsAbstract: false } && typeof(IAiProviderDriver).IsAssignableFrom(type))
            .ToList();

        Assert.Equal([typeof(AnthropicProviderDriver)], drivers);
    }

    // The point of the move: this family compiles against the contract and nothing else, and reaches neither the
    // shared provider runtime nor the product that hosts it. A reference to either would be one using directive
    // away and would make the family unbuildable outside this repository.
    [Fact]
    public void TheAddInReferencesTheContractAndNothingElseFromThisRepository()
    {
        var referenced = typeof(AnthropicProviderDriver).Assembly
    ...
    public void TheDriverImplementsTheContractsOwnInterface()
    {
        Assert.Same(
            ...
            typeof(AnthropicProviderDriver).GetInterfaces()
    ...
    {
        return new ConformanceSubject(new AnthropicProviderDriver())
        {
            Inputs = Stated,
        };
    }
