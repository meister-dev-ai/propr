...
/// <summary>
///     Pins what this family puts on the wire and what it makes of the answer, against a fake endpoint rather
///     than assumptions. The Messages API differs from the OpenAI family in three ways that a translation can
///     silently get wrong — the system prompt, tool results, and the cache buckets in its usage payload — so each
///     is asserted on the actual request body.
/// </summary>
public sealed class AnthropicMessagesChatClientTests
{
    private static readonly ProviderModelDescriptor Model =
        new(Guid.NewGuid(), "claude-opus-5", [ProviderDeclaredProtocolModes.Auto, AnthropicProviderDriver.MessagesProtocol]);
    ...
    [Fact]
    public async Task TheCallIsAddressedAtTheEndpointsOwnMessagesPath()
    {
        ...
        Assert.Equal("/v1/messages", Assert.Single(endpoint.Requests).RequestUri!.AbsolutePath);
    }
    ...
    // A connection that pins its own API version keeps it, and the header is sent once: two values would be a
    // request the provider cannot read.
    [Fact]
    public async Task AVersionTheConnectionPinsIsSentOnceAndIsTheOneThatTravels()
    {
        var endpoint = new FakeAnthropicEndpoint().Responds(TextResponse("ok"));
        var connection = Endpoint() with
        {
            DefaultHeaders = new Dictionary<string, string> { ["anthropic-version"] = "2024-10-22" },
        };

        await Client(endpoint, connection).GetResponseAsync([new ChatMessage(ChatRole.User, "hello")]);

        Assert.Equal("2024-10-22", Assert.Single(endpoint.Requests).Headers.GetValues("anthropic-version").Single());
    }
    ...
    [Fact]
    public async Task TheVendorsOwnCountsReachTheSeamWhereTheDriverMapsThem()
    {
        ...
        Assert.Equal(50, response.Usage.AdditionalCounts!["cache_creation_input_tokens"]);
    }
    ...
    // Anthropic requires at least one message. A conversation can empty out in translation without the caller
    // sending nothing: a system turn is carried in "system", and a turn whose blocks are all empty or of a kind
    // this driver does not translate is dropped. Refused here, with the status the vendor would have answered
    // with, so the classifier reads it as permanent and the cause names the host.
    [Theory]
    [InlineData("system-only")]
    [InlineData("empty-user-turn")]
    public async Task AConversationWithNothingToSendIsRefusedBeforeTheCall(string shape)
    {
        var endpoint = new FakeAnthropicEndpoint().Responds(TextResponse("ok"));
        ChatMessage[] messages = shape == "system-only"
            ? [new ChatMessage(ChatRole.System, "you are a reviewer")]
            : [new ChatMessage(ChatRole.User, string.Empty)];

        var failure = await Assert.ThrowsAsync<HttpRequestException>(() => Client(endpoint).GetResponseAsync(messages));
    ...
    // This family accepts any host speaking the Messages API, and a gateway in front of one states its error as
    // a string where the vendor states an object. The status still has to reach the classifier, because a 429 a
    // gateway raised is as retryable as one the vendor raised.
    [Theory]
    [InlineData("""{"error":"rate limited"}""")]
    [InlineData("""{"error":{"message":42}}""")]
    [InlineData("""{"error":["rate limited"]}""")]
    public async Task ARejectionWhoseErrorIsNotTheVendorsShapeStillCarriesItsStatus(string body)
    {
        var endpoint = new FakeAnthropicEndpoint().Fails(HttpStatusCode.TooManyRequests, body);
    ...
    [Fact]
    public async Task ARejectionStatingItsMessageDirectlyUnderErrorRepeatsThatMessage()
    {
        var endpoint = new FakeAnthropicEndpoint().Fails(
            HttpStatusCode.ServiceUnavailable,
            """{"error":"the gateway has no capacity"}""");

        var failure = await Assert.ThrowsAsync<HttpRequestException>(() => Client(endpoint).GetResponseAsync([new ChatMessage(ChatRole.User, "hello")]));

        Assert.Equal(HttpStatusCode.ServiceUnavailable, failure.StatusCode);
        Assert.Contains("the gateway has no capacity", failure.Message, StringComparison.Ordinal);
    }
    ...
    private static ProviderEndpoint Endpoint()
    {
        return new ProviderEndpoint(
            ...
            AnthropicProviderDriver.ApiKeyAuth,
            "sk-ant-key");
    }

    // Built over the host's client factory, which is the only way a family loaded from a directory reaches the
    // network at all.
    private static AnthropicMessagesChatClient Client(FakeAnthropicEndpoint wire, ProviderEndpoint? connection = null)
    {
        return new AnthropicMessagesChatClient(
            new FakeProviderHostContext(wire).Http,
            connection ?? Endpoint(),
            Model);
    }
