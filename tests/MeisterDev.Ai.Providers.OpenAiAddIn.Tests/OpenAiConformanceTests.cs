...
public sealed class OpenAiConformanceTests
{
    ...
    ///     the declaration. The vendor's own endpoint is what this family takes, and the Azure resource it has to
    ///     turn away is the rule that separates it from the family that serves that resource.
    /// </remarks>
    private static readonly ProviderConformanceInputs Stated = new(
        OpenAiProviderDriver.ApiKeyAuth,
    ...
    public void TheKitReportsEveryCheckItRan()
    {
        ...
        Assert.Equal("meisterdev/openAi", report.Family);
    ...
    {
        var declared = new OpenAiProviderDriver().Declaration.ConformanceInputs;
    ...
    {
        var constructed = typeof(OpenAiProviderDriver).GetConstructor(Type.EmptyTypes);

        Assert.NotNull(constructed);
        Assert.IsType<OpenAiProviderDriver>(constructed.Invoke([]));
    }
    ...
    {
        var drivers = typeof(OpenAiProviderDriver).Assembly
            .GetExportedTypes()
            .Where(type => type is { IsClass: true, IsAbstract: false } && typeof(IAiProviderDriver).IsAssignableFrom(type))
            .ToList();

        Assert.Equal([typeof(OpenAiProviderDriver)], drivers);
    }
    ...
    {
        var referenced = typeof(OpenAiProviderDriver).Assembly
    ...
    public void TheDriverImplementsTheContractsOwnInterface()
    {
        Assert.Same(
            ...
            typeof(OpenAiProviderDriver).GetInterfaces()
    ...
    {
        return new ConformanceSubject(new OpenAiProviderDriver())
        {
            Inputs = Stated,
        };
    }
