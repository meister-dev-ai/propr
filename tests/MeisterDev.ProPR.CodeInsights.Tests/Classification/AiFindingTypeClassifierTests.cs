...
public sealed class AiFindingTypeClassifierTests
{
    ...
    [Fact]
    public async Task ClassifyAsync_RecordsTheTokenSpendEvenWhenTheAnswerIsUnusable()
    {
        ...
        // Insight calls run outside a review job, so nothing else counts their tokens. Counting only the answers
        // that parsed would understate the bill by exactly the calls that went wrong.
