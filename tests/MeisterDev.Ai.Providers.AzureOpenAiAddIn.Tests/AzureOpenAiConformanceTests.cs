...
/// </remarks>
public sealed class AzureOpenAiConformanceTests
{
    /// <summary>
    ...
    ///     the declaration. This is the family that insists on an Azure AI host, so a plain vendor URL is what it
    ///     has to refuse, and the recorded payload is the shape this protocol reports: both totals already
    ///     contain their buckets, so a mapping that added either back would double-count it.
    /// </remarks>
    ...
    public void TheKitReportsEveryCheckItRan()
    {
        ...
        Assert.Equal("meisterdev/azureOpenAi", report.Family);
    ...
    {
        var declared = new AzureOpenAiProviderDriver().Declaration.ConformanceInputs;
    ...
    {
        var constructed = typeof(AzureOpenAiProviderDriver).GetConstructor(Type.EmptyTypes);

        Assert.NotNull(constructed);
        Assert.IsType<AzureOpenAiProviderDriver>(constructed.Invoke([]));
    }
    ...
    {
        var drivers = typeof(AzureOpenAiProviderDriver).Assembly
            .GetExportedTypes()
            .Where(type => type is { IsClass: true, IsAbstract: false } && typeof(IAiProviderDriver).IsAssignableFrom(type))
            .ToList();

        Assert.Equal([typeof(AzureOpenAiProviderDriver)], drivers);
    }

    // The point of the move: this family compiles against the contract and its own vendor dependencies, and
    // reaches neither the shared provider runtime nor the product that hosts it. A reference to either would be
    // one using directive away and would make the family unbuildable outside this repository. It also means the
    // Azure credential chain is this folder's business alone, and no longer a dependency of the shared provider
    // project that every other family compiles against.
    [Fact]
    public void TheAddInReferencesTheContractAndNothingElseFromThisRepository()
    {
        var referenced = typeof(AzureOpenAiProviderDriver).Assembly
    ...
    public void TheDriverImplementsTheContractsOwnInterface()
    {
        Assert.Same(
            ...
            typeof(AzureOpenAiProviderDriver).GetInterfaces()
    ...
    {
        return new ConformanceSubject(new AzureOpenAiProviderDriver())
        {
            Inputs = Stated,
        };
    }
