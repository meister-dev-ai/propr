...
/// </remarks>
public sealed class BedrockConformanceTests
{
    /// <summary>
    ...
    ///     the declaration. This is the family that insists on an AWS host naming a region, so a host that names
    ///     no region is what it has to refuse, and the recorded payload is the shape Converse reports: a prompt
    ///     count exclusive of both cache buckets, which a mapping that returned the vendor's counts unchanged
    ///     would bill at nothing.
    /// </remarks>
    ...
    public void TheKitReportsEveryCheckItRan()
    {
        ...
        Assert.Equal("meisterdev/awsBedrock", report.Family);
    ...
    {
        var declared = new BedrockProviderDriver().Declaration.ConformanceInputs;
    ...
    {
        var constructed = typeof(BedrockProviderDriver).GetConstructor(Type.EmptyTypes);

        Assert.NotNull(constructed);
        Assert.IsType<BedrockProviderDriver>(constructed.Invoke([]));
    }
    ...
    {
        var drivers = typeof(BedrockProviderDriver).Assembly
            .GetExportedTypes()
            .Where(type => type is { IsClass: true, IsAbstract: false } && typeof(IAiProviderDriver).IsAssignableFrom(type))
            .ToList();

        Assert.Equal([typeof(BedrockProviderDriver)], drivers);
    }
    ...
    // three AWS packages are this folder's business alone, so replacing the deprecated one touches nothing else.
    [Fact]
    public void TheAddInReferencesTheContractAndNothingElseFromThisRepository()
    {
        var referenced = typeof(BedrockProviderDriver).Assembly
    ...
    public void TheDriverImplementsTheContractsOwnInterface()
    {
        Assert.Same(
            ...
            typeof(BedrockProviderDriver).GetInterfaces()
    ...
    {
        return new ConformanceSubject(new BedrockProviderDriver())
        {
            Inputs = Stated,
        };
    }
