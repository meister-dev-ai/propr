...
/// <summary>
///     Integration tests for <see cref="MeisterDev.ProPR.Api.Controllers.ClientsController" />
    ...
    : IClassFixture<ClientsControllerTests.ClientsApiFactory>
{
    ...
    [Fact]
    public async Task PatchClient_EnableMultiPassUnion_PersistedAndReturned()
    {
        using var scope = factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<MeisterProPRDbContext>();
        var tenantId = Guid.NewGuid();
        db.Tenants.Add(CreateTenantRecord(tenantId, "multipass-test", "MultiPass Tenant"));
        var record = new ClientRecord
        {
            Id = Guid.NewGuid(),
            TenantId = tenantId,
            DisplayName = "MultiPass Test",
            IsActive = true,
            CreatedAt = DateTimeOffset.UtcNow,
        ...
        using var request = new HttpRequestMessage(HttpMethod.Patch, $"/clients/{record.Id}");
        request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", factory.GenerateAdminToken());
        request.Content = JsonContent.Create(new { enableMultiPassUnion = true });

        var response = await client.SendAsync(request);
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);

        var body = JsonDocument.Parse(await response.Content.ReadAsStringAsync());
        Assert.True(body.RootElement.GetProperty("enableMultiPassUnion").GetBoolean());
    ...
    [Fact]
    public async Task PatchClient_ReviewEveryIncrementEnabled_PersistedAndReturned()
    {
        using var scope = factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<MeisterProPRDbContext>();
        var tenantId = Guid.NewGuid();
        db.Tenants.Add(CreateTenantRecord(tenantId, "every-increment-test", "Every Increment Tenant"));
        var record = new ClientRecord
        {
            Id = Guid.NewGuid(),
            TenantId = tenantId,
            DisplayName = "Every Increment Test",
            IsActive = true,
            CreatedAt = DateTimeOffset.UtcNow,
        ...
        using var request = new HttpRequestMessage(HttpMethod.Patch, $"/clients/{record.Id}");
        request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", factory.GenerateAdminToken());
        request.Content = JsonContent.Create(new { reviewEveryIncrementEnabled = true });

        var response = await client.SendAsync(request);
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);

        var body = JsonDocument.Parse(await response.Content.ReadAsStringAsync());
        Assert.True(body.RootElement.GetProperty("reviewEveryIncrementEnabled").GetBoolean());

        db.ChangeTracker.Clear();
        var updated = await db.Clients.SingleAsync(c => c.Id == record.Id);
        Assert.True(updated.ReviewEveryIncrementEnabled);
    }
