...
public sealed class AiMemoryKeywordExtractorTests
{
    ...
    public async Task ExtractAsync_WithNoSummary_MakesNoCallAtAll()
    {
        ...
        var sut = new AiMemoryKeywordExtractor(resolver, Substitute.For<IModelUsageRecorder>(), NullLogger<AiMemoryKeywordExtractor>.Instance);
    ...
    {
        var resolver = Substitute.For<IAiRuntimeResolver>();
        resolver.ResolveChatRuntimeAsync(ClientId, AiPurpose.MemoryReconsideration, Arg.Any<CancellationToken>())
            .ThrowsAsync(new AiPurposeBindingNotConfiguredException(AiPurpose.MemoryReconsideration));
