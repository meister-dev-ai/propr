...
public sealed class ProCursorOperationalPersistenceOwnershipTests
{
    ...
    {
        var contextContents = File.ReadAllText(Path.Combine(RepoRoot, "src/MeisterDev.ProPR.Infrastructure/Data/MeisterProPRDbContext.cs"));
    ...
    {
        Assert.False(File.Exists(Path.Combine(RepoRoot, "src/MeisterDev.ProPR.Infrastructure/Data/ProCursorOperationalDbContext.cs")));
        Assert.False(
            File.Exists(
                Path.Combine(RepoRoot, "src/MeisterDev.ProPR.Infrastructure/Features/UsageReporting/Persistence/ProCursorTokenUsageReadRepository.cs")));
        Assert.False(
            File.Exists(Path.Combine(RepoRoot, "src/MeisterDev.ProPR.Infrastructure/Features/UsageReporting/Persistence/EfProCursorTokenUsageRecorder.cs")));
        Assert.False(
            File.Exists(
                Path.Combine(RepoRoot, "src/MeisterDev.ProPR.Infrastructure/Features/UsageReporting/Services/ProCursorTokenUsageAggregationService.cs")));
        Assert.False(
            File.Exists(Path.Combine(RepoRoot, "src/MeisterDev.ProPR.Infrastructure/Features/UsageReporting/Services/ProCursorTokenUsageRebuildService.cs")));
        Assert.False(
            File.Exists(Path.Combine(RepoRoot, "src/MeisterDev.ProPR.Infrastructure/Features/UsageReporting/Services/ProCursorTokenUsageRetentionService.cs")));
    }
