...
public sealed class StoreAnthropicRowsUnderItsDeclaredNamesMigrationTests(PostgresContainerFixture fixture)
{
    ...
    public async Task Upgrading_NamesTheFamilysRowsAsItDeclaresThemAndLeavesAnotherFamilyAlone()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            await using (var beforeMove = CreateDbContext(scratch))
            {
                ...
                var clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Anthropic Identity Move");

                await SeedConnectionAsync(beforeMove, moved, clientId, LegacyIdentity, "ApiKey", "AnthropicMessages");

                // The other family moved before this one, so its rows already hold its own qualified names. It
                // owns the credential shape this family also declares, which the identity on the row is
                // there to tell apart.
            ...
            await using (var afterMove = CreateDbContext(scratch))
            {
                ...
                Assert.Equal(
                    $"{DeclaredIdentity}:AnthropicMessages",
                ...
                Assert.Equal(
                    $"Auto|{DeclaredIdentity}:AnthropicMessages|Embeddings",
                    await ListedProtocolModesAsync(afterMove, moved));

                // Another family's rows are not touched by this family's migration.
                Assert.Equal(OtherIdentity, await IdentityAsync(afterMove, untouched));
                Assert.Equal($"{OtherIdentity}:ApiKey", await AuthModeAsync(afterMove, untouched));
                Assert.Equal($"{OtherIdentity}:ChatCompletions", await BoundProtocolModeAsync(afterMove, untouched));
                Assert.Equal(
                    $"Auto|{OtherIdentity}:ChatCompletions|Embeddings",
                    await ListedProtocolModesAsync(afterMove, untouched));
    ...
    // This family declares two credential shapes: the one the provider actually reads its key from, and the
    // plain key mode a profile saved before that was declared still holds. Both have to move, or a profile on
    // the one left behind quarantines on the credential axis once the family owns its vocabulary.
    [Fact]
    public async Task Upgrading_MovesBothCredentialShapesTheFamilyDeclares()
    {
        fixture.SkipIfUnavailable();

        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            var plainKey = new SeededConnection();
            var headerKey = new SeededConnection();

            await using (var beforeMove = CreateDbContext(scratch))
            {
                await MigrateToAsync(beforeMove, BeforeMove);
                var clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Anthropic Credential Shapes");

                await SeedConnectionAsync(beforeMove, plainKey, clientId, LegacyIdentity, "ApiKey", "AnthropicMessages", "Plain key profile");
                await SeedConnectionAsync(beforeMove, headerKey, clientId, LegacyIdentity, "XApiKey", "AnthropicMessages", "Header key profile");
            }
            ...
            await using (var afterMove = CreateDbContext(scratch))
            {
                ...
                // Both spellings land on the one shape the family declares: they named the same single value and
                // produced the same request, so the row that said XApiKey is not a different configuration.
                Assert.Equal($"{DeclaredIdentity}:ApiKey", await AuthModeAsync(afterMove, plainKey));
                Assert.Equal($"{DeclaredIdentity}:ApiKey", await AuthModeAsync(afterMove, headerKey));
            }
        });
    }
    ...
    public async Task Upgrading_FoldsADailySampleOntoTheOneAlreadyWrittenUnderTheDeclaredIdentity()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            {
                await MigrateToAsync(beforeMove, BeforeMove);
                clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Anthropic Usage Fold");

                await SeedUsageSampleAsync(beforeMove, clientId, "claude-opus-5", "reviewer", LegacyIdentity, 100, 4m);
                await SeedUsageSampleAsync(beforeMove, clientId, "claude-opus-5", "reviewer", DeclaredIdentity, 30, 1m);

                // A sample with no counterpart moves rather than folds, and one nothing priced stays unpriced.
                await SeedUsageSampleAsync(beforeMove, clientId, "claude-haiku-5", "triage", LegacyIdentity, 7, null);
                await SeedUsageSampleAsync(beforeMove, clientId, "gpt-5", "reviewer", OtherIdentity, 55, 9m);
            }
            ...
            await using (var afterMove = CreateDbContext(scratch))
            {
                ...
                Assert.Equal(1, await SampleCountAsync(afterMove, clientId, "claude-opus-5"));
                Assert.Equal(130L, await SampleInputTokensAsync(afterMove, clientId, "claude-opus-5", DeclaredIdentity));
                Assert.Equal(5m, await SampleCostAsync(afterMove, clientId, "claude-opus-5", DeclaredIdentity));

                Assert.Equal(7L, await SampleInputTokensAsync(afterMove, clientId, "claude-haiku-5", DeclaredIdentity));
                Assert.Null(await SampleCostAsync(afterMove, clientId, "claude-haiku-5", DeclaredIdentity));
                ...
                Assert.Equal(55L, await SampleInputTokensAsync(afterMove, clientId, "gpt-5", OtherIdentity));
            }
        });
    }
    ...
    public async Task Reverting_PutsEveryRowBackUnderTheNamesTheFamilySuperseded()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            {
                await MigrateToAsync(beforeMove, BeforeMove);
                clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Anthropic Identity Revert");
            ...
            await using (var reverted = CreateDbContext(scratch))
            {
                ...
                Assert.Equal("AnthropicMessages", await BoundProtocolModeAsync(reverted, moved));
                Assert.Equal("Auto|AnthropicMessages|Embeddings", await ListedProtocolModesAsync(reverted, moved));
                ...
                Assert.Equal(1, await SampleCountAsync(reverted, clientId, "claude-opus-5"));
                Assert.Equal(130L, await SampleInputTokensAsync(reverted, clientId, "claude-opus-5", LegacyIdentity));
                Assert.Equal(5m, await SampleCostAsync(reverted, clientId, "claude-opus-5", LegacyIdentity));
            }
        });
    }
        ...
        string? displayName = null)
    {
        ...
        await dbContext.Database.ExecuteSqlRawAsync(
            ...
            VALUES ({0}, {1}, {2}, {3}, 'https://api.anthropic.com/v1', {4}, 'Manual',
    ...
    {
        var databaseName = $"propr_anthropic_identity_migration_{Guid.NewGuid():N}";
