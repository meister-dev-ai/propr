...
public sealed class StoreGoogleVertexRowsUnderItsDeclaredNamesMigrationTests(PostgresContainerFixture fixture)
{
    ...
    public async Task Upgrading_NamesTheFamilysRowsAsItDeclaresThemAndLeavesAnotherFamilyAlone()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            await using (var beforeMove = CreateDbContext(scratch))
            {
                ...
                var clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Vertex Identity Move");

                await SeedConnectionAsync(beforeMove, moved, clientId, LegacyIdentity, "GcpAdc", "GoogleGenerateContent");
            ...
            await using (var afterMove = CreateDbContext(scratch))
            {
                ...
                Assert.Equal($"{DeclaredIdentity}:GcpAdc", await AuthModeAsync(afterMove, moved));
                Assert.Equal(
                    $"{DeclaredIdentity}:GoogleGenerateContent",
                ...
                Assert.Equal(
                    $"Auto|{DeclaredIdentity}:GoogleGenerateContent|Embeddings",
    ...
    public async Task Upgrading_FoldsADailySampleOntoTheOneAlreadyWrittenUnderTheDeclaredIdentity()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            {
                await MigrateToAsync(beforeMove, BeforeMove);
                clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Vertex Usage Fold");

                await SeedUsageSampleAsync(beforeMove, clientId, "gemini-2.5-pro", "reviewer", LegacyIdentity, 100, 4m);
                await SeedUsageSampleAsync(beforeMove, clientId, "gemini-2.5-pro", "reviewer", DeclaredIdentity, 30, 1m);

                // A sample with no counterpart moves rather than folds, and one nothing priced stays unpriced.
                await SeedUsageSampleAsync(beforeMove, clientId, "gemini-2.5-flash", "triage", LegacyIdentity, 7, null);
            ...
            await using (var afterMove = CreateDbContext(scratch))
            {
                ...
                Assert.Equal(1, await SampleCountAsync(afterMove, clientId, "gemini-2.5-pro"));
                Assert.Equal(130L, await SampleInputTokensAsync(afterMove, clientId, "gemini-2.5-pro", DeclaredIdentity));
                Assert.Equal(5m, await SampleCostAsync(afterMove, clientId, "gemini-2.5-pro", DeclaredIdentity));

                Assert.Equal(7L, await SampleInputTokensAsync(afterMove, clientId, "gemini-2.5-flash", DeclaredIdentity));
                Assert.Null(await SampleCostAsync(afterMove, clientId, "gemini-2.5-flash", DeclaredIdentity));
    ...
    public async Task Reverting_PutsEveryRowBackUnderTheNamesTheFamilySuperseded()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            {
                await MigrateToAsync(beforeMove, BeforeMove);
                clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Vertex Identity Revert");
            ...
            await using (var reverted = CreateDbContext(scratch))
            {
                ...
                Assert.Equal("GcpAdc", await AuthModeAsync(reverted, moved));
                Assert.Equal("GoogleGenerateContent", await BoundProtocolModeAsync(reverted, moved));
                Assert.Equal("Auto|GoogleGenerateContent|Embeddings", await ListedProtocolModesAsync(reverted, moved));
                ...
                Assert.Equal(1, await SampleCountAsync(reverted, clientId, "gemini-2.5-pro"));
                Assert.Equal(130L, await SampleInputTokensAsync(reverted, clientId, "gemini-2.5-pro", LegacyIdentity));
                Assert.Equal(5m, await SampleCostAsync(reverted, clientId, "gemini-2.5-pro", LegacyIdentity));
            }
        });
    }
        ...
        string ownedProtocolMode)
    {
        ...
        await dbContext.Database.ExecuteSqlRawAsync(
            ...
            VALUES ({0}, {1}, {2}, {3}, 'https://europe-west4-aiplatform.googleapis.com', {4}, 'Manual',
    ...
    {
        var databaseName = $"propr_vertex_identity_migration_{Guid.NewGuid():N}";
