...
public sealed class StoreAwsBedrockRowsUnderItsDeclaredNamesMigrationTests(PostgresContainerFixture fixture)
{
    ...
    public async Task Upgrading_NamesTheFamilysRowsAsItDeclaresThemAndLeavesAnotherFamilyAlone()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            await using (var beforeMove = CreateDbContext(scratch))
            {
                ...
                var clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Bedrock Identity Move");

                await SeedConnectionAsync(beforeMove, moved, clientId, LegacyIdentity, "SigV4", "BedrockConverse");
                await SeedConnectionAsync(beforeMove, untouched, clientId, OtherIdentity, "XApiKey", "ChatCompletions");
            ...
            await using (var afterMove = CreateDbContext(scratch))
            {
                ...
                Assert.Equal($"{DeclaredIdentity}:SigV4", await AuthModeAsync(afterMove, moved));
                Assert.Equal(
                    $"{DeclaredIdentity}:BedrockConverse",
                ...
                Assert.Equal(
                    $"Auto|{DeclaredIdentity}:BedrockConverse|Embeddings",
                ...
                Assert.Equal("XApiKey", await AuthModeAsync(afterMove, untouched));
                Assert.Equal("ChatCompletions", await BoundProtocolModeAsync(afterMove, untouched));
                Assert.Equal("Auto|ChatCompletions|Embeddings", await ListedProtocolModesAsync(afterMove, untouched));
    ...
    // This family declares two credential shapes. Every new profile is SigV4; a profile saved before the
    // credential fields were declared still holds the packed pair under the plain API-key shape, and it has to
    // move too or that profile quarantines on the credential axis once the family owns its vocabulary.
    [Fact]
    public async Task Upgrading_MovesBothCredentialShapesTheFamilyDeclares()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            var signed = new SeededConnection();
            var packed = new SeededConnection();

            await using (var beforeMove = CreateDbContext(scratch))
            {
                await MigrateToAsync(beforeMove, BeforeMove);
                var clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Bedrock Credential Shapes");

                await SeedConnectionAsync(beforeMove, signed, clientId, LegacyIdentity, "SigV4", "BedrockConverse", "Signed profile");
                await SeedConnectionAsync(beforeMove, packed, clientId, LegacyIdentity, "ApiKey", "BedrockConverse", "Packed profile");
            }
            ...
            await using (var afterMove = CreateDbContext(scratch))
            {
                ...
                Assert.Equal($"{DeclaredIdentity}:SigV4", await AuthModeAsync(afterMove, signed));
                Assert.Equal($"{DeclaredIdentity}:ApiKey", await AuthModeAsync(afterMove, packed));
            }
        });
    }
    ...
    public async Task Upgrading_FoldsADailySampleOntoTheOneAlreadyWrittenUnderTheDeclaredIdentity()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            {
                await MigrateToAsync(beforeMove, BeforeMove);
                clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Bedrock Usage Fold");

                await SeedUsageSampleAsync(beforeMove, clientId, "anthropic.claude-opus-4-5", "reviewer", LegacyIdentity, 100, 4m);
                await SeedUsageSampleAsync(beforeMove, clientId, "anthropic.claude-opus-4-5", "reviewer", DeclaredIdentity, 30, 1m);

                // A sample with no counterpart moves rather than folds, and one nothing priced stays unpriced.
                await SeedUsageSampleAsync(beforeMove, clientId, "amazon.titan-text", "triage", LegacyIdentity, 7, null);
                await SeedUsageSampleAsync(beforeMove, clientId, "claude-opus-4-5", "reviewer", OtherIdentity, 55, 9m);
            }
            ...
            await using (var afterMove = CreateDbContext(scratch))
            {
                ...
                Assert.Equal(1, await SampleCountAsync(afterMove, clientId, "anthropic.claude-opus-4-5"));
                Assert.Equal(130L, await SampleInputTokensAsync(afterMove, clientId, "anthropic.claude-opus-4-5", DeclaredIdentity));
                Assert.Equal(5m, await SampleCostAsync(afterMove, clientId, "anthropic.claude-opus-4-5", DeclaredIdentity));

                Assert.Equal(7L, await SampleInputTokensAsync(afterMove, clientId, "amazon.titan-text", DeclaredIdentity));
                Assert.Null(await SampleCostAsync(afterMove, clientId, "amazon.titan-text", DeclaredIdentity));
                ...
                Assert.Equal(55L, await SampleInputTokensAsync(afterMove, clientId, "claude-opus-4-5", OtherIdentity));
            }
        });
    }
    ...
    public async Task Reverting_PutsEveryRowBackUnderTheNamesTheFamilySuperseded()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            {
                await MigrateToAsync(beforeMove, BeforeMove);
                clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Bedrock Identity Revert");
            ...
            await using (var reverted = CreateDbContext(scratch))
            {
                ...
                Assert.Equal("SigV4", await AuthModeAsync(reverted, moved));
                Assert.Equal("BedrockConverse", await BoundProtocolModeAsync(reverted, moved));
                Assert.Equal("Auto|BedrockConverse|Embeddings", await ListedProtocolModesAsync(reverted, moved));
                ...
                Assert.Equal(1, await SampleCountAsync(reverted, clientId, "anthropic.claude-opus-4-5"));
                Assert.Equal(130L, await SampleInputTokensAsync(reverted, clientId, "anthropic.claude-opus-4-5", LegacyIdentity));
                Assert.Equal(5m, await SampleCostAsync(reverted, clientId, "anthropic.claude-opus-4-5", LegacyIdentity));
            }
        });
    }
        ...
        string? displayName = null)
    {
        ...
        await dbContext.Database.ExecuteSqlRawAsync(
            ...
            VALUES ({0}, {1}, {2}, {3}, 'https://bedrock-runtime.eu-central-1.amazonaws.com', {4}, 'Manual',
    ...
    {
        var databaseName = $"propr_bedrock_identity_migration_{Guid.NewGuid():N}";
