...
public sealed class StoreAzureOpenAiRowsUnderItsDeclaredNamesMigrationTests(PostgresContainerFixture fixture)
{
    ...
    public async Task Upgrading_NamesTheFamilysRowsAsItDeclaresThemAndLeavesAnotherFamilyAlone()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            await using (var beforeMove = CreateDbContext(scratch))
            {
                ...
                var clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Azure Identity Move");

                await SeedConnectionAsync(beforeMove, moved, clientId, LegacyIdentity, "ApiKey", "Responses");
                await SeedConnectionAsync(beforeMove, untouched, clientId, OtherIdentity, "ApiKey", "ChatCompletions");
    ...
    // This family declares two credential shapes: a stored resource key, and a managed identity that stores no
    // credential at all. Both have to move, or a profile on the one left behind quarantines on the credential
    // axis once the family owns its vocabulary.
    [Fact]
    public async Task Upgrading_MovesBothCredentialShapesTheFamilyDeclares()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            var keyed = new SeededConnection();
            var managedIdentity = new SeededConnection();

            await using (var beforeMove = CreateDbContext(scratch))
            {
                await MigrateToAsync(beforeMove, BeforeMove);
                var clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Azure Credential Shapes");

                await SeedConnectionAsync(beforeMove, keyed, clientId, LegacyIdentity, "ApiKey", "Responses", "Key profile");
                await SeedConnectionAsync(beforeMove, managedIdentity, clientId, LegacyIdentity, "AzureIdentity", "Responses", "Managed identity profile");
            }
            ...
            await using (var afterMove = CreateDbContext(scratch))
            {
                ...
                Assert.Equal($"{DeclaredIdentity}:ApiKey", await AuthModeAsync(afterMove, keyed));
                Assert.Equal($"{DeclaredIdentity}:AzureIdentity", await AuthModeAsync(afterMove, managedIdentity));
            }
        });
    }
    ...
    public async Task Upgrading_FoldsADailySampleOntoTheOneAlreadyWrittenUnderTheDeclaredIdentity()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            {
                await MigrateToAsync(beforeMove, BeforeMove);
                clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Azure Usage Fold");
    ...
    public async Task Reverting_PutsEveryRowBackUnderTheNamesTheFamilySuperseded()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            {
                await MigrateToAsync(beforeMove, BeforeMove);
                clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Azure Identity Revert");
        ...
        string? displayName = null)
    {
        ...
        await dbContext.Database.ExecuteSqlRawAsync(
            ...
            VALUES ({0}, {1}, {2}, {3}, 'https://contoso.openai.azure.com', {4}, 'Manual',
    ...
    {
        var databaseName = $"propr_azureopenai_identity_migration_{Guid.NewGuid():N}";
