...
public sealed class StoreLiteLlmRowsUnderItsDeclaredNamesMigrationTests(PostgresContainerFixture fixture)
{
    ...
    public async Task Upgrading_NamesTheFamilysRowsAsItDeclaresThemAndLeavesAnotherFamilyAlone()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            await using (var beforeMove = CreateDbContext(scratch))
            {
                ...
                var clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "LiteLLM Identity Move");

                await SeedConnectionAsync(beforeMove, moved, clientId, LegacyIdentity, "ApiKey", "ChatCompletions");
                await SeedConnectionAsync(beforeMove, untouched, clientId, OtherIdentity, "ApiKey", "Responses");
            ...
            await using (var afterMove = CreateDbContext(scratch))
            {
                ...
                Assert.Equal(
                    $"{DeclaredIdentity}:ChatCompletions",
                ...
                Assert.Equal("Responses", await BoundProtocolModeAsync(afterMove, untouched));
                Assert.Equal("Auto|Responses|Embeddings", await ListedProtocolModesAsync(afterMove, untouched));
    ...
    // This family owns two wire shapes, and one of them is a shape another family owns as well. The identity on
    // the row is what tells them apart, so the same stored value moves on one connection and stays on the other.
    [Fact]
    public async Task Upgrading_MovesTheSharedWireShapeOnlyOnThisFamilysConnections()
    {
        fixture.SkipIfUnavailable();
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            {
                await MigrateToAsync(beforeMove, BeforeMove);
                var clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "LiteLLM Shared Shape");
            ...
            await using (var afterMove = CreateDbContext(scratch))
            {
                ...
                Assert.Equal($"{DeclaredIdentity}:Responses", await BoundProtocolModeAsync(afterMove, moved));
                Assert.Equal(
    ...
    public async Task Upgrading_FoldsADailySampleOntoTheOneAlreadyWrittenUnderTheDeclaredIdentity()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            {
                await MigrateToAsync(beforeMove, BeforeMove);
                clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "LiteLLM Usage Fold");

                await SeedUsageSampleAsync(beforeMove, clientId, "claude-opus-4-5", "reviewer", LegacyIdentity, 100, 4m);
                await SeedUsageSampleAsync(beforeMove, clientId, "claude-opus-4-5", "reviewer", DeclaredIdentity, 30, 1m);

                // A sample with no counterpart moves rather than folds, and one nothing priced stays unpriced.
                await SeedUsageSampleAsync(beforeMove, clientId, "gpt-5", "triage", LegacyIdentity, 7, null);
                await SeedUsageSampleAsync(beforeMove, clientId, "gpt-4o", "reviewer", OtherIdentity, 55, 9m);
            }
            ...
            await using (var afterMove = CreateDbContext(scratch))
            {
                ...
                Assert.Equal(1, await SampleCountAsync(afterMove, clientId, "claude-opus-4-5"));
                Assert.Equal(130L, await SampleInputTokensAsync(afterMove, clientId, "claude-opus-4-5", DeclaredIdentity));
                Assert.Equal(5m, await SampleCostAsync(afterMove, clientId, "claude-opus-4-5", DeclaredIdentity));

                Assert.Equal(7L, await SampleInputTokensAsync(afterMove, clientId, "gpt-5", DeclaredIdentity));
                Assert.Null(await SampleCostAsync(afterMove, clientId, "gpt-5", DeclaredIdentity));
                ...
                Assert.Equal(55L, await SampleInputTokensAsync(afterMove, clientId, "gpt-4o", OtherIdentity));
            }
        });
    }
    ...
    public async Task Reverting_PutsEveryRowBackUnderTheNamesTheFamilySuperseded()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            {
                await MigrateToAsync(beforeMove, BeforeMove);
                clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "LiteLLM Identity Revert");
            ...
            await using (var reverted = CreateDbContext(scratch))
            {
                ...
                Assert.Equal("ChatCompletions", await BoundProtocolModeAsync(reverted, moved));
                Assert.Equal("Auto|ChatCompletions|Embeddings", await ListedProtocolModesAsync(reverted, moved));
                ...
                Assert.Equal(1, await SampleCountAsync(reverted, clientId, "claude-opus-4-5"));
                Assert.Equal(130L, await SampleInputTokensAsync(reverted, clientId, "claude-opus-4-5", LegacyIdentity));
                Assert.Equal(5m, await SampleCostAsync(reverted, clientId, "claude-opus-4-5", LegacyIdentity));
            }
        });
    }
        ...
        string ownedProtocolMode)
    {
        ...
        await dbContext.Database.ExecuteSqlRawAsync(
            ...
            VALUES ({0}, {1}, {2}, {3}, 'https://gateway.example.com/v1', {4}, 'Manual',
                    CAST({5} AS jsonb), CAST({5} AS jsonb), true, now(), now());
            """,
            [seeded.ConnectionId, clientId, $"Seeded {providerKind}", providerKind, authMode, "{}"]);
    ...
    {
        var databaseName = $"propr_litellm_identity_migration_{Guid.NewGuid():N}";
