...
public sealed class StoreOpenAiCompatibleRowsUnderItsDeclaredNamesMigrationTests(PostgresContainerFixture fixture)
{
    ...
    public async Task Upgrading_NamesTheFamilysRowsAsItDeclaresThemAndLeavesAnotherFamilyAlone()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            await using (var beforeMove = CreateDbContext(scratch))
            {
                ...
                var clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Compatible Identity Move");
    ...
    public async Task Upgrading_FoldsADailySampleOntoTheOneAlreadyWrittenUnderTheDeclaredIdentity()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            {
                await MigrateToAsync(beforeMove, BeforeMove);
                clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Compatible Usage Fold");

                await SeedUsageSampleAsync(beforeMove, clientId, "kimi-k2", "reviewer", LegacyIdentity, 100, 4m);
                await SeedUsageSampleAsync(beforeMove, clientId, "kimi-k2", "reviewer", DeclaredIdentity, 30, 1m);

                // A sample with no counterpart moves rather than folds, and one nothing priced stays unpriced.
                await SeedUsageSampleAsync(beforeMove, clientId, "gpt-oss-120b", "triage", LegacyIdentity, 7, null);
            ...
            await using (var afterMove = CreateDbContext(scratch))
            {
                ...
                Assert.Equal(1, await SampleCountAsync(afterMove, clientId, "kimi-k2"));
                Assert.Equal(130L, await SampleInputTokensAsync(afterMove, clientId, "kimi-k2", DeclaredIdentity));
                Assert.Equal(5m, await SampleCostAsync(afterMove, clientId, "kimi-k2", DeclaredIdentity));

                Assert.Equal(7L, await SampleInputTokensAsync(afterMove, clientId, "gpt-oss-120b", DeclaredIdentity));
                Assert.Null(await SampleCostAsync(afterMove, clientId, "gpt-oss-120b", DeclaredIdentity));
    ...
    public async Task Reverting_PutsEveryRowBackUnderTheNamesTheFamilySuperseded()
    {
        ...
        await this.OnAScratchDatabaseAsync(async scratch =>
        {
            ...
            {
                await MigrateToAsync(beforeMove, BeforeMove);
                clientId = await HistoricalSchemaSeed.SeedClientAsync(beforeMove, "Compatible Identity Revert");
            ...
            await using (var reverted = CreateDbContext(scratch))
            {
                ...
                Assert.Equal(1, await SampleCountAsync(reverted, clientId, "kimi-k2"));
                Assert.Equal(130L, await SampleInputTokensAsync(reverted, clientId, "kimi-k2", LegacyIdentity));
                Assert.Equal(5m, await SampleCostAsync(reverted, clientId, "kimi-k2", LegacyIdentity));
            }
        });
    }
        ...
        string ownedProtocolMode)
    {
        ...
        await dbContext.Database.ExecuteSqlRawAsync(
            ...
            VALUES ({0}, {1}, {2}, {3}, 'https://opencode.ai/zen/v1', {4}, 'Manual',
    ...
    {
        var databaseName = $"propr_compatible_identity_migration_{Guid.NewGuid():N}";
