...
public sealed class OpenAiCompatibleConformanceTests
{
    ...
    ///     the declaration. This family exists to accept the long tail, so an endpoint belonging to no vendor is
    ///     what it has to take and there is nothing for it to refuse on account of the host in it.
    /// </remarks>
    private static readonly ProviderConformanceInputs Stated = new(
        OpenAiCompatibleProviderDriver.ApiKeyAuth,
    ...
    public void TheKitReportsEveryCheckItRan()
    {
        ...
        Assert.Equal("meisterdev/openAiCompatible", report.Family);
    ...
    {
        var declared = new OpenAiCompatibleProviderDriver().Declaration.ConformanceInputs;
    ...
    {
        var constructed = typeof(OpenAiCompatibleProviderDriver).GetConstructor(Type.EmptyTypes);

        Assert.NotNull(constructed);
        Assert.IsType<OpenAiCompatibleProviderDriver>(constructed.Invoke([]));
    }
    ...
    {
        var drivers = typeof(OpenAiCompatibleProviderDriver).Assembly
            .GetExportedTypes()
            .Where(type => type is { IsClass: true, IsAbstract: false } && typeof(IAiProviderDriver).IsAssignableFrom(type))
            .ToList();

        Assert.Equal([typeof(OpenAiCompatibleProviderDriver)], drivers);
    }
    ...
    {
        var referenced = typeof(OpenAiCompatibleProviderDriver).Assembly
    ...
    public void TheDriverImplementsTheContractsOwnInterface()
    {
        Assert.Same(
            ...
            typeof(OpenAiCompatibleProviderDriver).GetInterfaces()
    ...
    {
        return new ConformanceSubject(new OpenAiCompatibleProviderDriver())
        {
            Inputs = Stated,
        };
    }
