...
///     host's, and it sits on the pipeline every OpenAI-shaped family goes out on, so what it has to survive is
///     the library's own deserializer and not any one family's construction of a client.
/// </remarks>
public sealed class FinishReasonWireBehaviourTests
{
    ...
    ///     with or without the repair on it.
    /// </summary>
    /// <param name="endpoint">What the pipeline sends to.</param>
    /// <param name="withRepair">Whether the finish-reason repair sits between the library and the endpoint.</param>
    private static IChatClient Client(FakeCompatibleEndpoint endpoint, bool withRepair = false)
    {
        HttpMessageHandler pipeline = withRepair
            ? new FinishReasonNormalizingHandler { InnerHandler = endpoint }
            : endpoint;
        ...
        return new OpenAiChatClient("gpt-5.6-luna", new ApiKeyCredential("key"), options).AsIChatClient();
    }
}
