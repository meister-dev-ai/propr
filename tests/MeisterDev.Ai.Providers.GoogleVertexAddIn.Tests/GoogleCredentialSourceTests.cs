...
public sealed class GoogleCredentialSourceTests
{
    ...
    public async Task TheGeminiApiTakesTheStoredKeyAsAHeaderAndBuildsNoCredential()
    {
        ...
        await new GoogleCredentialSource().AuthenticateAsync(
            request,
            Endpoint(GeminiBaseUrl, "a-gemini-api-key", GoogleVertexProviderDriver.ApiKeyAuth));
    ...
    // The header is read as a key by whatever is at the other end, so what goes into it must be one. A profile
    // configured for a Google credential document holds a private key, and putting that in a header would send
    // it to an endpoint that cannot use it and has no business seeing it.
    [Fact]
    public async Task ACredentialDocumentIsNeverWrittenIntoTheGeminiApiKeyHeader()
    {
        const string serviceAccount = "{\"type\":\"service_account\",\"private_key\":\"super-secret-material\"}";
        using var request = new HttpRequestMessage(HttpMethod.Post, GeminiBaseUrl);

        var failure = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            new GoogleCredentialSource().AuthenticateAsync(
                request,
                Endpoint(GeminiBaseUrl, serviceAccount, GoogleVertexProviderDriver.GcpAdcAuth)));

        Assert.False(request.Headers.Contains(GoogleCredentialSource.ApiKeyHeaderName));
        Assert.Null(request.Headers.Authorization);
    ...
    private static ProviderEndpoint Endpoint(string baseUrl, string? secret, string mode = GoogleVertexProviderDriver.GcpAdcAuth)
    {
        return new ProviderEndpoint("meisterdev/googleVertex", baseUrl, mode, secret);
    }
}
