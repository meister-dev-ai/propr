...
/// </remarks>
public sealed class GoogleVertexConformanceTests
{
    /// <summary>
    ...
    ///     the declaration, so this suite states the inputs beside the checks and asserts the declaration
    ///     matches them.
    /// </remarks>
    private static readonly ProviderConformanceInputs Stated = new(
        GoogleVertexProviderDriver.GcpAdcAuth,

        // Google's prompt count already contains the cached portion, and its thinking tokens sit outside the
        // candidate count.
    ...
    public void TheKitReportsEveryCheckItRan()
    {
        ...
        Assert.Equal("meisterdev/googleVertex", report.Family);
    ...
    {
        var declared = new GoogleVertexProviderDriver().Declaration.ConformanceInputs;
    ...
    {
        var constructed = typeof(GoogleVertexProviderDriver).GetConstructor(Type.EmptyTypes);

        Assert.NotNull(constructed);
        Assert.IsType<GoogleVertexProviderDriver>(constructed.Invoke([]));
    }
    ...
    {
        var drivers = typeof(GoogleVertexProviderDriver).Assembly
            .GetExportedTypes()
            .Where(type => type is { IsClass: true, IsAbstract: false } && typeof(IAiProviderDriver).IsAssignableFrom(type))
            .ToList();

        Assert.Equal([typeof(GoogleVertexProviderDriver)], drivers);
    }
    ...
    {
        var referenced = typeof(GoogleVertexProviderDriver).Assembly
    ...
    public void TheDriverImplementsTheContractsOwnInterface()
    {
        Assert.Same(
            ...
            typeof(GoogleVertexProviderDriver).GetInterfaces()
    ...
    {
        return new ConformanceSubject(new GoogleVertexProviderDriver())
        {
            Inputs = Stated,
        };
    }
