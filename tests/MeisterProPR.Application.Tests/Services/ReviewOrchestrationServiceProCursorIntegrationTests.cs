...
public sealed class ReviewOrchestrationServiceProCursorIntegrationTests
{
    ...
    public async Task ProcessAsync_WhenReviewUsesProCursorKnowledgeTool_DelegatesThroughGatewayBackedReviewTools()
    {
        ...
        await proCursorGateway.Received(1).AskKnowledgeAsync(
            Arg.Is<ProCursorKnowledgeQueryRequest>(request =>
                ...
                request.RepositoryContext.OrganizationUrl == job.OrganizationUrl &&
                request.RepositoryContext.ProjectId == job.ProjectId &&
    ...
    public async Task ProcessAsync_WhenReviewUsesProCursorSymbolTool_DelegatesThroughGatewayBackedReviewTools()
    {
        ...
        await proCursorGateway.Received(1).GetSymbolInsightAsync(
        ...
        IProCursorGateway proCursorGateway)
    {
        ...
        instructionFetcher.FetchAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<Guid?>(), Arg.Any<CancellationToken>())
        ...
        instructionEvaluator.EvaluateRelevanceAsync(Arg.Any<IReadOnlyList<RepositoryInstruction>>(), Arg.Any<IReadOnlyList<string>>(), Arg.Any<CancellationToken>())
            .Returns(Task.FromResult<IReadOnlyList<RepositoryInstruction>>([]));
    ...
    private static PullRequest CreatePullRequest(ReviewJob job)
    {
        return new PullRequest(
            ...
            "main",
            new List<ChangedFile> { new("/src/Greeter.cs", ChangeType.Edit, "class Greeter {}", "@@ -1 +1 @@", false) }.AsReadOnly(),
            PrStatus.Active,
            null);
    }
}
