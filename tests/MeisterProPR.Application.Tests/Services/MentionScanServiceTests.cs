...
public sealed class MentionScanServiceTests
{
    ...
    public async Task ScanAsync_NoPrsSinceWatermark_SkipsThreadFetch()
    {
        ...
        this._scanRepository.GetProjectScanAsync(ConfigId).ReturnsForAnyArgs(
            new MentionProjectScan(Guid.NewGuid(), ConfigId, DateTimeOffset.UtcNow));
        this._activePrFetcher.GetRecentlyUpdatedPullRequestsAsync(
            Arg.Any<string>(), Arg.Any<string>(), Arg.Any<DateTimeOffset>(), Arg.Any<Guid?>(), Arg.Any<CancellationToken>())
        ...
        await this._pullRequestFetcher.DidNotReceiveWithAnyArgs().FetchAsync(
    ...
    public async Task ScanAsync_PrWithOlderTimestampThanWatermark_SkipsPr()
    {
        ...
        var pr = new ActivePullRequestRef("https://dev.azure.com/org", "proj", "repo", 42,
        ...
        this._scanRepository.GetPrScanAsync(ConfigId, "repo", 42).ReturnsForAnyArgs(
            new MentionPrScan(Guid.NewGuid(), ConfigId, "repo", 42, lastSeen));
    ...
    public async Task ScanAsync_MentionFoundAndNotDuplicate_EnqueuesJob()
    {
        ...
        var pr = new ActivePullRequestRef("https://dev.azure.com/org", "proj", "repo", 1,
            DateTimeOffset.UtcNow);
        ...
        var pullRequest = new PullRequest(
            "https://dev.azure.com/org", "proj", "repo", 1, 1,
            "Test PR", "desc", "feature/x", "main", [],
        ...
        this._jobRepository.ExistsForCommentAsync(
            ClientId, 1, 100, 200, Arg.Any<CancellationToken>()).Returns(false);

        // Act
        await this._sut.ScanAsync();

        // Assert: job was added to repo and channel
        await this._jobRepository.Received(1).AddAsync(
    ...
    public async Task ScanAsync_MentionAlreadyProcessed_DoesNotEnqueueDuplicate()
    {
        ...
        var pullRequest = new PullRequest(
            "https://dev.azure.com/org", "proj", "repo", 1, 1,
            "PR", null, "b", "main", [], ExistingThreads: [thread]);
        ...
        await this._jobRepository.DidNotReceiveWithAnyArgs().AddAsync(
            Arg.Any<MentionReplyJob>(), Arg.Any<CancellationToken>());
    ...
    public async Task ScanAsync_AfterCycle_UpsertsBothWatermarks()
    {
        ...
        var pr = new ActivePullRequestRef("https://dev.azure.com/org", "proj", "repo", 5,
            DateTimeOffset.UtcNow);
        var pullRequest = new PullRequest(
            "https://dev.azure.com/org", "proj", "repo", 5, 1,
            "Empty PR", null, "b", "main", [], ExistingThreads: []);
        ...
        await this._scanRepository.Received(1).UpsertProjectScanAsync(
            ...
            Arg.Any<CancellationToken>());
        await this._scanRepository.Received(1).UpsertPrScanAsync(
