...
public sealed class ProCursorKnowledgeQueryServiceTests
{
    ...
    public async Task AskKnowledgeAsync_WithRepositoryContextBias_ReturnsOrderedMatches()
    {
        ...
        var repoSnapshot = CreateReadySnapshot(
            repositorySource,
            repositorySource.TrackedBranches.Single(),
            "commit-repo");
        ...
        var result = await service.AskKnowledgeAsync(
            new ProCursorKnowledgeQueryRequest(
                ...
                RepositoryContext: new ProCursorRepositoryContextDto(
                    repositorySource.ProviderScopePath,
                    repositorySource.ProviderProjectKey,
        ...
        await embeddingService.Received(1)
            .GenerateEmbeddingsAsync(
                ...
                Arg.Is<IReadOnlyList<string>>(inputs =>
                    inputs.Count == 1 && inputs[0].Contains("token caching", StringComparison.OrdinalIgnoreCase)),
    ...
    public async Task AskKnowledgeAsync_WhenSnapshotLagsBranchHead_ReturnsStaleStatus()
    {
        ...
        var symbolGraphRepository = Substitute.For<IProCursorSymbolGraphRepository>();
        var source = CreateSource("App Repository", ProCursorSourceKind.Repository, "repo-app", "commit-newer");
        ...
        var chunk = CreateChunk(
            snapshot.Id,
            "src/Auth/TokenCache.cs",
            "TokenCache",
            "Token caching reduces redundant network calls against Azure DevOps.");
    ...
    public async Task AskKnowledgeAsync_WhenEligibleSourcesExceedConfiguredLimit_OnlyScansTopSources()
    {
        ...
        snapshotRepository.ListKnowledgeChunksAsync(firstSnapshot.Id, Arg.Any<CancellationToken>())
            .Returns(
                Task.FromResult<IReadOnlyList<ProCursorKnowledgeChunk>>(
                [
    ...
    public async Task AskKnowledgeAsync_WhenNoChunksMatch_ReturnsNoResult()
    {
        ...
        var chunk = CreateChunk(
            ...
            "docs/auth.md",
            "Authentication",
            "This document explains JWT claims and PAT validation.");
    ...
    private static IProCursorEmbeddingService CreateUnavailableEmbeddingService()
    {
        ...
        embeddingService.GenerateEmbeddingsAsync(
                ...
                Arg.Any<CancellationToken>())
            .Returns(_ =>
                Task.FromException<IReadOnlyList<float[]>>(new InvalidOperationException("no_embedding_connection_configured")));
