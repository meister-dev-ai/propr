...
public sealed class AgenticFileByFileReviewOrchestratorRepeatedJudgmentTests
{
    [Fact]
    public async Task ReviewAsync_UnresolvedAgenticFinding_ReusesFixedEvidenceForRepeatedJudgmentAndPublishesAgreement()
    {
        ...
        const string investigationJson =
            """
            {
              ...
              "evidence_set_id": "evidence-task-001",
              "evidence": [
                {
                  "kind": "file_content",
        ...
        const string prVerificationJson =
            ...
            {
              "verdict": "unresolved",
              "recommended_disposition": "SummaryOnly",
              "reason_codes": ["missing_verified_claim_support"],
              "summary": "Repository evidence was suggestive but not decisive."
            }
            """;

        const string repeatedJudgmentJson =
            """
            {
              "verdict": "supported",
              "recommended_disposition": "Publish",
              "reason_codes": ["verified_bounded_claim_support"],
              "summary": "The same fixed evidence still supports the DI registration concern."
            }
            """;
        ...
        chatClient.GetResponseAsync(Arg.Any<IList<ChatMessage>>(), Arg.Any<ChatOptions?>(), Arg.Any<CancellationToken>())
            .Returns(
                ...
                new ChatResponse(new ChatMessage(ChatRole.Assistant, repeatedJudgmentJson)));
        ...
        await protocolRecorder.Received()
            .RecordReviewStrategyEventAsync(
                ...
                Arg.Is<string?>(details => details != null
                                           && details.Contains("\"findingId\":\"candidate-001\"", StringComparison.Ordinal)
                                           && details.Contains("\"evidenceSetId\":\"evidence-task-001\"", StringComparison.Ordinal)),
                Arg.Is<string?>(output => output != null
                                          && output.Contains("\"agreementState\":\"Agreed\"", StringComparison.Ordinal)
                                          && output.Contains("\"recommendedDisposition\":\"Publish\"", StringComparison.Ordinal)
                                          && output.Contains("\"usedSameEvidenceSet\":true", StringComparison.Ordinal)),
