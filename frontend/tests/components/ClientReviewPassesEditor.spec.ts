...
describe('ClientReviewPassesEditor', () => {
  ...
  it('adds a pass and emits a contiguous list once a model is chosen', async () => {
    ...
    expect(lastEmit(wrapper)).toEqual([{ ordinal: 0, configuredModelId: 'model-a1', lens: null, scope: null, shadow: false }])
  })
  ...
  it('emits contiguous ordinals for multiple passes', async () => {
    ...
    expect(lastEmit(wrapper)).toEqual([
      { ordinal: 0, configuredModelId: 'model-a1', lens: null, scope: null, shadow: false },
      { ordinal: 1, configuredModelId: 'model-b1', lens: null, scope: null, shadow: false },
    ])
  })
  ...
  it('removes a pass and re-indexes the remaining ordinals', async () => {
    ...
    expect(lastEmit(wrapper)).toEqual([{ ordinal: 0, configuredModelId: 'model-b1', lens: null, scope: null, shadow: false }])
  })
  ...
  it('reorders passes with the move controls', async () => {
    ...
    expect(lastEmit(wrapper)).toEqual([
      { ordinal: 0, configuredModelId: 'model-b1', lens: null, scope: null, shadow: false },
      { ordinal: 1, configuredModelId: 'model-a1', lens: null, scope: null, shadow: false },
    ])
  })
