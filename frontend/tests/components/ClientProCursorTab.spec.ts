...
async function mountTab() {
  const { default: ClientProCursorTab } = await import('@/features/clients/components/ClientProCursorTab.vue')
...
describe('ClientProCursorTab', () => {
  ...
  it('shows an actionable error state when ProCursor loading fails', async () => {
    listSourcesMock.mockRejectedValue(new Error('Failed to load ProCursor sources.'))

    const wrapper = await mountTab()
    await flushPromises()

    expect(listSourcesMock).toHaveBeenCalledWith('client-1')
