...
function onConfigSaved(saved: WebhookConfigurationResponse) {
  ...
  if (index >= 0) {
    ...
    selectedConfigId.value = saved.id ?? null
    deliveries.value = []
...
function describeEvents(events: WebhookEventType[] | null | undefined): string {
...
function describeFilters(filters: WebhookRepoFilterResponse[] | null | undefined): string {
